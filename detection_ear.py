...
for dlib_rect, landmark in zip(dlib_rects, list_landmarks):
    ...
    x = landmark[30][0] -dlib_rect.width()//4 # 이미지에서 코 부위의 x값
    y = landmark[30][1] - dlib_rect.height()//6 # 이미지에서 코 부위의 y값 - 얼굴 영역의 세로를 차지하는 픽셀의 수//2
    # y = landmark[30][1] - dlib_rect.height()//2
    w = h = dlib_rect.width() # 얼굴 영역의 가로를 차지하는 픽셀의 수 (531-345+1) → max(x) - min(x) +1(픽셀의 수 이기 때문에 1을 더해줌 → 픽셀 수는 점 하나로도 1이 됨)
    print (f'(x,y) : ({x},{y})')
    print (f'(w,h) : ({w},{h})')

sticker_dir = r"C:\Users\Jennie\Desktop\aiffel\Aiffel_MiniProject7\images\bow (1).png"
...
refined_x = x - w//2 # 440 - (322//2) = 440-161 =279 (refined_x = x - w//2)
...
rotation = cv2.getRotationMatrix2D((sticker_width/2, sticker_height/2), 8, 1)
